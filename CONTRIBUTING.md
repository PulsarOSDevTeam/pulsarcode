...
The mechanisms in this launcher compose with technology that is **not** in this repository. Specifically, the PulsarOS Intelligence Inc. patent portfolio (`.col` model compression, AETHER substrate, PHSL runtime, RELAY iPhone companion, BACKBONE skill protocol) is upstream. We coordinate the launcher tightly with those upstream pieces. An open coordination process would dilute that coordination. The launcher is small, AGPL-licensed, and forkable; the upstream is sovereign.

---

Copyright (C) 2026 PulsarOS Intelligence Inc. / Collapse Technologies Inc. AGPL-3.0-or-later. See `LICENSE`.
